classdef MultisensorExtractor < Appliable & UnSupervisedTrainable & Uncertainty
    ...
    methods
        ...
        function infoCell = info(this)
            infoCell = [];
            for i = 1:length(this.extractors) %size(this.extractors,2)
        ...
        function U = uncertainty(this, U_x, data)
            U = [];
            for i = 1 : size(U_x,2)
                U = [U this.extractors{i}.uncertainty(U_x{1,i},data{1,i})];
            end
        end
    end
end
