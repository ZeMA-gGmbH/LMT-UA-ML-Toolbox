classdef LDAEuklidClassifier < CLSuperClass & Uncertainty
    ...
    methods
        ...
        function train(this, X, Y)
            ...
            %%% Test without Mahal
            testCell = cell(size(groups,1),1);
            for i = 1:size(groups,1)
                testCell{i} = eye(min(size(X,2),dim),min(size(X,2),dim));
            end
            this.icovar = testCell;
            %cellfun(@(a){​​inv(proj' * (a * proj))}​​, covarianz);
            %%%
        ...
        function showAxisData(this)
            ...
            for i = 1:size(this.data_plot,1)
                ...
                %plot becomes more clear
                    ...
                    scat = scatter(proj(:,1),proj(:,2),30,this.target_plot{i,1},'o','MarkerEdgeColor',[i/numD 0 0],'LineWidth',0.5);
                end
                hold on
            end
            box on
        ...
        function showUncertainty(this,U,data) %warum ist das hier das Target?
            figure()
            box on
            hold on
            first = true;
